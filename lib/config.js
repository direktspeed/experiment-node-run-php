(function () {
    ...
    var Config = function (source) {
        ...
        if (options.http.vhosts == true) {
            var dirname = options.http.vhostsDir + '/' || '/app/config/domains/'
            var filename = ""           
            var CurrentSettings = ""
            this.vhosts = {}
            ...
            fs.readdir(dirname, function(err, filenames) {
                ...
                for (i=0; i < filenames.length; i++) {
                  ...
                  for (j=0; j < Object.keys(CurrentSettings).length; j++) {
                    this.vhosts[Object.keys(CurrentSettings)[j]] = CurrentSettings[Object.keys(CurrentSettings)[j]]
    ...
    Config.prototype.prepareVhosts = function (folder) {
    ...
    Config.prototype.prepareApp = function (appconfig) {
        console.log('DEPRECATED! app in config is depricated use vhost or http options')
        return appconfig
    }
    ...
    Config.prototype.prepareHttp = function (http) {
        ...
        if (http.vhosts == true) {
            var dirname = http.vhostsDir + '/' || '/app/config/domains/'
            var filename = ""           
            var CurrentSettings = ""
            http.vhosts = {}
            ...
            fs.readdir(dirname, function(err, filenames) {
                ...
                for (i=0; i < filenames.length; i++) {
                  ...
                  for (j=0; j < Object.keys(CurrentSettings).length; j++) {
                    try {
                        ...
                        http.vhosts[Object.keys(CurrentSettings)[j]] = CurrentSettings[Object.keys(CurrentSettings)[j]]
                        
                        /* Prepare express apps and use them as object attached to the http.vhost */

                        // http.vhosts[Object.keys(CurrentSettings)[j]].aO = 
                        // require(CurrentSettings[Object.keys(CurrentSettings)[j]].vhost_use)(CurrentSettings[Object.keys(CurrentSettings)[j]].app)
                        
                        // console.log(CurrentSettings[Object.keys(CurrentSettings)[j]].app)
                        // CurrentSettings[Object.keys(CurrentSettings)[j]] = null
                    } catch(e) { 
                        console.log(Object.keys(CurrentSettings)[j], e) 
                    }
                ...
                if (i == filenames.length -1) console.log('Success: Loading Domain Settings ' + dirname)
                };
                // callback(0, Settings);
            }.bind(this))
            // Instance all apps under the name of them and then use them later inside the Request handlers.
    ...
    Config.prototype.prepareBackends = function (backends) {
    ...
    Config.prototype.prepareUser = function (user) {
        ...
        if ( (process.getuid() === 0) || (! /^win/.test(process.platform)) ) {
            ...
            console.log('Running under Windows as Administrator!')
        }
        return user;
    };
