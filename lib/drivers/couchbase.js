(function () {
    ...
    try {
        var SubDriver = require('couchbase');
    } catch (e) {
        // XXX new logging infrastructure not landed yet
    ...
    var cbc = function () {
        ...
        var drivers = this.drivers.map(function (u) {
            // XXX new logging infrastructure not landed yet
            // if (u.auth) {
            //     logger.error('cbcached', 'This driver doesn\'t support authentication!!!');
            // }
            // TODO support unix sockets binding
            if (u.port == 'undefined') return (u.hostname || '127.0.0.1');
                else return (u.hostname || '127.0.0.1') + ':' + (u.port);
        });
        // The principal client
        // var clientReady = false;
        var client = new SubDriver.Cluster(drivers);
        ...
        this.create = function (host, vhost, callback) {
            client.upsert(prefix + 'frontend:' + host, [vhost], 0, callback);
        };
        ...
        this.getHttpCache = function (hosts, callback) {
            var first = hosts[0];
            hosts = hosts.map(function (host) {
                return prefix + 'frontend:' + host;
            });
        ...
        this.saveHttpCache = function (hosts, callback) {
            var first = hosts[0];
            hosts = hosts.map(function (host) {
                return prefix + 'frontend:' + host;
            });
