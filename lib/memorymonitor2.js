...
function MemoryMonitor(options) {
    ...
    options = options || {};
    options.memoryLimit = options.memoryLimit || 200; // 200MB per worker when used with php you need to add the php max mem here
    options.gracefulWait = options.gracefulWait || 120; // 120 seconds wait till a worker ends even when it uses to much memory
